...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 SPC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zSPC send
enum ZerocoinSpendStatus {
    ZSPC_SPEND_OKAY = 0,                            // No error
    ZSPC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSPC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSPC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSPC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSPC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSPC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSPC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSPC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSPC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSPC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSPC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSPC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSPC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSPC_SPENT_USED_ZSPC = 14,                      // Coin has already been spend
    ZSPC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSPC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZSPCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzSPCTracker> zpivTracker;
    ...
    void setZWallet(CzSPCWallet* zwallet);
    CzSPCWallet* getZWallet();
    ...
    /// Get 10000 SPC output and keys which can be used for the Masternode
    ...
    /** zSPC reset */
    boost::signals2::signal<void()> NotifyzSPCReset;
