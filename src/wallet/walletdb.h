...
public:
    ...
    bool WriteZSPCSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZSPCSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZSPCSeed_deprecated(uint256& seed);
    bool EraseZSPCSeed();
    bool EraseZSPCSeed_deprecated();

    bool WriteZSPCCount(const uint32_t& nCount);
    bool ReadZSPCCount(uint32_t& nCount);
