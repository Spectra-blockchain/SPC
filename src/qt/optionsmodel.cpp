...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::SPC);
    ...
    if (settings.contains("nAnonymizeSpectraAmount") || reset)
        SoftSetArg("-anonymizespectraamount", settings.value("nAnonymizeSpectraAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in spectra.cpp during shotdown to also remove the window positions
