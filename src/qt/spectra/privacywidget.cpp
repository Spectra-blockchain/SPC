...
#include "qt/spectra/privacywidget.h"
#include "qt/spectra/forms/ui_privacywidget.h"
#include "qt/spectra/qtutils.h"
#include "guiutil.h"
#include "qt/spectra/denomgenerationdialog.h"
#include "qt/spectra/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(SpectraGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zSPC anonymizes your SPC by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zSPC or convert back to SPC"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 SPC ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zSPC");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zSPC");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zSPC");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zSPC");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zSPC");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zSPC");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zSPC");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zSPC");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zSPC Movements"));
    ...
    // Only Convert to SPC enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzSPC->setTitleClassAndText("btn-title-grey", "Total 0 zSPC");
    ui->btnTotalzSPC->setSubTitleClassAndText("text-subtitle", "Show denominations of zSPC owned.");
    ui->btnTotalzSPC->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select SPC outputs to mint into zSPC.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zSPC");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzSPC, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zSPC");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of SPC to mint into zSPC"));
    }else{
        btnText = tr("Convert back to SPC");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zSPC to convert back into SPC"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzSPC->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzSPC->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zSPC is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zSPC, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zSPC").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zSPC minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zSPC converted back to SPC"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any SPC to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSPC </b>";
    ...
    ui->btnTotalzSPC->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
