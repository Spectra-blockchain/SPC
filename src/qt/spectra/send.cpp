...
#include "qt/spectra/send.h"
#include "qt/spectra/forms/ui_send.h"
#include "qt/spectra/addnewcontactdialog.h"
#include "qt/spectra/qtutils.h"
#include "qt/spectra/sendchangeaddressdialog.h"
#include "qt/spectra/optionbutton.h"
#include "qt/spectra/sendconfirmdialog.h"
#include "qt/spectra/myaddressrow.h"
#include "qt/spectra/guitransactionsutils.h"
...
SendWidget::SendWidget(SpectraGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (SPC)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("Spectra address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 SPC");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_SPC : AskPassphraseDialog::Context::Send_zSPC, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zSPC is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zSPC selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zSPC transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZSPC_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zSPC require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zSPC transaction
        ...
        emit message("zSPC transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isSPC){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zSPC in your balance to select."));
        }
    }
}
...
void SendWidget::onSPCSelected(bool _isSPC){
    isSPC = _isSPC;
    setCssProperty(coinIcon, _isSPC ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
