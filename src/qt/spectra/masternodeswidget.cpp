...
#include "qt/spectra/forms/ui_masternodeswidget.h"
#include "qt/spectra/qtutils.h"
#include "qt/spectra/mnrow.h"
#include "qt/spectra/mninfodialog.h"

#include "qt/spectra/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(SpectraGUI *parent) :
    PWidget(parent),
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= CollateralRequired(chainActive.Height())){
            ...
            // kind of like "...node, %i SPC requited").arg(qYourCollateralResultVariable),
            // to nicely print required collateral amount.
            inform(tr("Not enough balance to create a masternode")); //, 10,000 SPC required."));
            return;
        }
        showHideOp(true);
