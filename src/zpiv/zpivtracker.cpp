// Copyright (c) 2018-2020 The Spectra developers
...
CzSPCTracker::CzSPCTracker(std::string strWalletFile)
{
...
CzSPCTracker::~CzSPCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSPCTracker::Init()
{
...
bool CzSPCTracker::Archive(CMintMeta& meta)
{
...
bool CzSPCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSPCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSPCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSPCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzSPCTracker::GetSerialHashes()
{
...
CAmount CzSPCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSPCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSPCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSPCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSPCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSPCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSPCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSPCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSPCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSPCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSPCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzSPCWallet* zSPCWallet)
{
    bool iszSPCWalletInitialized = (NULL != zSPCWallet);
    CMintMeta meta;
    ...
    if (! iszSPCWalletInitialized)
        zSPCWallet = new CzSPCWallet(strWalletFile);
    meta.isSeedCorrect = zSPCWallet->CheckSeed(dMint);
    if (! iszSPCWalletInitialized)
        delete zSPCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzSPCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSPCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSPCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSPCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSPCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSPCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzSPCWallet* zSPCWallet = new CzSPCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zSPCWallet);
        }
        delete zSPCWallet;
