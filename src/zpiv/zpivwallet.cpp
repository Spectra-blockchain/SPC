// Copyright (c) 2017-2019 The Spectra developers
...
CzSPCWallet::CzSPCWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSPCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSPCSeed_deprecated()) {
                    LogPrintf("%s: Updated zSPC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzSPCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSPCCount(nCountLastUsed);
    else if (!walletdb.ReadZSPCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSPCWallet::Lock()
{
    seedMaster = 0;
}

void CzSPCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSPCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSPC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSPCWallet::LoadMintPoolFromDB()
{
...
void CzSPCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSPCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSPCWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzSPCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSPC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzSPCWallet::SeedToZSPC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSPCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSPCWallet::GenerateDeterministicZSPC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSPCWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSPCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzSPCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
