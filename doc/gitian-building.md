...
*Setup instructions for a gitian build of Spectra Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Spectra
...
to the Spectra GitHub release page.
...
- [Building Spectra Core](#building-spectra-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/spectra-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/spectra-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/spectra-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/spectra-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/spectra-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/spectra-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://https://github.com/Spectra-blockchain/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@spectrablockchain.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/spectra-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/spectra-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/spectra-osx-*-build.assert
```
